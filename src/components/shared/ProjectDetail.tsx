...
import TechStack, { TechTool } from '@/components/shared/TechStack'
...
export function ProjectDetail({
                  ...
                  >
                    <ReactMarkdown
                      components={{
                        p: ({ children }) => <>{children}</>,
                        strong: ({ children }) => (
                          <strong className="font-bold text-gray-140">{children}</strong>
                        ),
                      }}
                    >
                      {detail}
  ...
  const renderLinkButton = (link: ProjectLink, key: string) => {
    const button = (
      <Button
        ...
        className={clx(
          ...
          (link.disabled || !link.href) && 'hover:text-gray-110 text-gray-130 cursor-not-allowed',
          link.className,
        )}
      >
        {renderLinkIcon(link)}
    ...
    return (
      <Link key={key} href={link.href} target="_blank" rel="noopener noreferrer">
        {button}
      </Link>
    )
  }
  ...
  return (
    ...
    >
      <div ref={ref} className="max-w-3xl mx-auto p-6 lg:pt-0 space-y-6">
        ...
        >
          <Image src={projectImage} alt={projectName} width={3000} height={2000} quality={100} />
        </div>
        ...
        {linkGroups?.map((group, groupIndex) => (
          <div key={`group-${groupIndex}`} className="w-full">
            {group.title && <h2 className="font-bold text-2xl pt-2 pb-2">{group.title}</h2>}
            <div className={clx('flex flex-wrap gap-4 py-6 items-center', group.className)}>
        ...
        {solutionsHeader && (
          <section>
            ...
            {solutionsConclusion && (
              <p className="text-lg text-gray-140 pt-2">{solutionsConclusion}</p>
            )}
          </section>
        )}

        {techStacks?.map((stack) => (
          <TechStack key={stack.title} title={stack.title} techStackTool={stack.tools} />
        ))}
