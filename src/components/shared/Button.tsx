...
export interface ButtonProps extends ComponentProps<'button'>, VariantProps<typeof button_cva> {
  asChild?: boolean
  iconOnly?: boolean
}
...
const button_icon_cva = cva('block stroke-inherit text-inherit stroke-[1.5px] shrink-0', {
  variants: {
    size: {
      small: 'h-4 w-4',
      medium: 'h-5 w-5',
...
const ButtonIcon: ForwardRefExoticComponent<ButtonIconProps> = forwardRef(
  ({ children, className, ...props }, ref) => {
    ...
    return (
      <Slot ref={ref} className={button_icon_cva({ size, className })} {...props}>
        {children}
      </Slot>
    )
  },
